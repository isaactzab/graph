...
=================================================
Kelvin R. Lawrence <gfxman@yahoo.com>, Traducción al Español por Isaac Levi Tzab <levi@tzab.mx> 
...
.Este libro es un trabajo en progreso. ¡La retroalimentación es muy incentivante y bienvenida!

El título de este libro podría ser igualmente "Una guía de inicio para usuarios de
bases de datos de Gráfos y el lenguaje de consulta de Gremlin con sugerencias, consejos 
y muestras consultas "'. Resulta que es demasiado largo para caber en una línea para un
encabezado pero en una sola oración que describe el enfoque de este trabajo bastante bien.

Me he resistido a la necesidad de tratar de cubrir cada característica de TinkerPop en un 
manual de referencia una tras otra vez. En cambio, lo que he intentado hacer es capturar
el proceso de aprendizaje por el que yo mismo he pasado usando lo que espero sea sensato
fluya desde el comienzo hasta temas más avanzados. Para aprovechar al máximo este libro,
recomiendo abra la consola Gremlin usando mis datos de muestrav a lo largo de su lectura.
Asumo que el lector no tiene conocimientos previos de Apache TinkerPop, el lenguaje de 
consulta Gremlin o herramientas las relacionadas. Comezaré a presentar todo lo necesario
comenzar en el Capítulo 2.

Espero que la gente encuentre útil lo que sigue. Definitivamente sigue siendo un trabajo
en progreso y se agregará más en las próximas semanas y meses según lo permita el tiempo.
Tengo la esperanza de que lo que se presenta hasta ahora es útil para cualquiera que, 
como yo, esté aprendiendo a usar Gremlin Query, el lenguaje transversal y las tecnologías 
relacionadas.

Una gran cantidad de material adicional, incluido el libro en muchos formatos diferentes,
como PDF, HTML, ePub y MOBI, así como código de muestra y datos, se pueden encontrar en
https://github.com/krlawrence/graph la página de inicio del proyecto en GitHub]. 
Encontrarás un resumen de todo lo que está disponible en la sección "<<samplesintro>>".

De donde surgió este libro
~~~~~~~~~~~~~~~~~~~~~~~~~~

No recuerdo exactamente cuando fue, pero en algun momento del 2016 comencé a compilar
una lista de notas, claves y consejos, inicialmente para mi propio beneficio. Mis
notas estaban llenas de cosas que tuve que encontrar pobremente explicadas en otras
partes mientras utilizaba una base de datos de Gráfos y especialmente mientras utilizaba
Apache TinkerPop, Gremlin y JanusGraph. Con el tiempo, el documento creció y efectivamente
se convirtió en el libro sin nombre. Luego de un poco de ánimo de mis colegas decidí 
publicar mis notas como un "living book" en formato de codigo abierto para que cualquiera
interesado pueda leerlo. Definitivamente esta enfocado a programadores e científicos
de datos, pero creo que puede ser leido por cualquiera que utiliza Gremlin graph query y
el lenguaje traversal para trabajar con bases de datos de Gráfos.

Incluyo una larga lista de codigos de ejemplos y muestras de consultas junto con debates
sobre las buenas prácticas, así como algunas lecciones que he aprendido, que estoy
seguro que encontrarás informativas. Le he llamado un "living book" por que mi objetivo
principal es mantener actualizaciones conforme vaya descubriendo mas cosas que deberian 
agregarse mientras que también a medida que Apache TinkerPop vaya evolucionando, mantener
el contenido actualizado.

!Quiero ser sincero diciendo "Gracias" a todos aquellos que me han animado a continuar
con esta aventura!. Ha tomado mucho trabajo pero todavia queda "mucho por divertirnos"

Kelvin R. Lawrence +
Primer Borrador: 5 de Octubre de 2017 +
Borrador Actual: {draftdate} +

Retroalimentación
~~~~~~~~~~~~~~~~~

Porfavor, avisame si encuentras en este material errores; de igual manera sientete
libre de enviar cualquier tipo de retroaliementación. Sugerencias y mejoras son 
muy bien recibidas. Una buena forma de enviar retroalimentación es abrir un "issue"
en el repositorio de GitHub que se encuentra en http://github.com/krlawrence/graph. 
Actualmente estas leyendo este libro en la revision {revision}.

Agradezco a quienes se han tomado el tiempo para revisar el manuscrito y han abierto
"issues" o han enviado "pull requests".
...
[[about]]
¿De qué tráta este libro?
~~~~~~~~~~~~~~~~~~~~~~~~~

Este libro es una introduccón a Apache TinkerPop 3 'Gremlin' graph query y lenguaje
traversal, por medio de ejemplos realistas y características que los datos de Gráfos
tendrían en el mundo real. Estos datos junto con los códigos de prueba y aplicaciones
de ejemplo estan disponibles para descargar en el proyecto de GitHub así como muchos otros
elementos. El grafo, 'air-routes', es un modelo de red de rutas de aerolíneas en el 
mundo que comprende alrededor de 3,373 aeropuertos incluyendo 43,400 rutas. El emplo
que presentamos funcionará sin que tener que modificarlo cargando en la console de Gremlin el archivo `air-routes.graphml` y con ThinkerGraph funcionando. El 'cómo configura el ambiente' es un tema cubierto en la seccion posterior <<gremlininstall>>.

NOTE: Los ejemplos de este libro fueron probados utilizando la version {tpvercheck} Apache TinkerPop. Sin embargo, el trabajo continúa agregando nuevas características que seran lanzadas en el libro.

ThinkerGrap es un grafo 'in-memmory', que quiere decir que nada se guarda automáticamente en el disco. Se entrega como parte de la descarga de Apache TinkerPop 3. El objetivo principal de este tutorial es permitir que alguien con poco o nulo conocimiento pueda levantar y utilizar rápidamente la consola de Gremlin y el grafo 'air-routes'. Más adelante, utilizando tecnologías como JanusGraph, Apache Cassandra, Gremlin Server y Elasticsearch, se discute cómo construir un almacenamiento de datos que sea escalable y persistente almacen de Gráfos que pueda ser atravezado utilizando consultas con Gremlin. También se discutirá sobre Java Standalone y las aplicaciones Groovy utilizando la consola Gremlin. Incluso también he agregado un par de ejemplos utilizando Ruby.

NOTE: En las primeras secciones de este libro me enfoqué principalmente en mostrar los diferentes tipos de consultas que pueden ser realizadas utilizando Gremlin. No intenté mostrar todos los posibles resultados de cada una de estas consultas, pero los ejemplos mostrados han sido bien seleccionados. Se profundizará en estos aspectos en los capítulos 4, 5 y 6.

.Cómo se organiza este libro.
Capítulo 1 - INTRODUCCIÓN::
- Se comienza haciendo una breve recapitulación de por qué nos interesa una Base de datos de Gráfos y se discute unos buenos casos de uso. También se provee de enlaces a programas de prueba y otros recursos adicionales referenciados por este libro.
Capítulo 2 - INICIANDO::
- En el Capítulo dos se presentan varios componentes de Apache TinkerPop así ocomo el archivo  `air-routes.graphml` en el cual se basa la mayor parte de los ejemplos de este libro.}
Capítulo 3 - ESCRIBIENDO CONSULTAS EN GREMLIN::
- En el Capítulo tres se ponen las cosas mas interesantes! Comenzamos a discutir sobre cómo utilizar el traversal Gremlin graph y el lenguaje de consulta para interrogar al grafo 'air-routes' utilizando una base de datos más tradicional y entonces mirando como SQL y Gremlin tienen similitudes en muchos aspectos y al mismo tiempo son diferentes en otros. El resto del capítulo se presentan varios métodos claves de Gremlin o de cómo son llamados a menudo '"Steps"'. En este capítulo enfoco principalmente en la lectura del grafo (sin agregar o remover cosas).
Capítulo 4 - MÁS ALLÁ DE UNA SIMPLE CONSULTA::
- En el Capítulo cuatro el enfoque se mueve más alla de solemente leer el grafo y describimos como agregar vértices (nodos), arístas (edges) y propiedades, así de cómo actualizarlos y eliminarlos. También se presenta una discusión de diversas buenas prácticas. En este capítulo también comenzamos a explorar algunos tópicos mas avanzados.
Capítulo 5 - ALGUNAS CONSULTAS Y LOS RESULTADOS QUE GENERAN::
- En el Capítulo cinco nos enfocamos en lo que se cubrión en capítulos anteriores para poder escribir consultas que tengan más sentido en el mundo real. Se presentan muchos ejemplos del resultado de ejecutar las consultas de este capítulo. También se comienzan a discutir tópicos como el análisis de distancias, distribución de rutas y de cómo escribir consultas geoespaciales.

Capítulo 6 - MOVIENDONOS A TRAVÉS DE LA CONSOLA Y THINKERGRAPH::
- In Capítulo six I start to expand the focus to concepts beyond using the Gremlin
  ...
  and indexes.
Capítulo 7 - INTRODUCING GREMLIN SERVER::
- In Capítulo seven, Gremlin Server is introduced. We begin to explore connecting to 
  and working with a remote graph both from the Gremlin Console and the command line
  as well as from code.
Capítulo 8 - COMMON GRAPH SERIALIZATION FORMATS::
- In Capítulo seven a discussion is presented of some common Graph serialization file
  formats along with coverage of how to use them in the context of TinkerPop 3
  enabled graphs.
Capítulo 9 - FURTHER READING::
...
[[samplesintro]]
Presentando las fuentes, programas ejemplos y datos utilizados en el libro
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Todo el trabajo relacionado con este proyecto se realiza de forma abierta en GitHub.
Abajo, se provee de una lista donde encontrarás los componentes claves. Los ejemplos
en este libro utilizan un grafo de prueba llamado 'air-routes' que contiene un grafo
basado en la red de mas de 3,370 aeropuertos y aerolineas del mundo. El grafo de ejemplo,
tienen bastante codigo de muestra y algunas aplicaciones de ejemplo pueden ser
encontradas en la misma ubicacion de GitHub que almacena el libro.
En la misma ubicación encontrarás lanzamientos del libro en diversos formatos (HTML,
PDF, DocBook/XML, MOBI y EPUB). Los programas de ejemplo incluyen Java (autónomo),
Groovy y Ruby asi como otros muchos ejemplos que pueden funcionar en la consola de Gremlin.
Hay algunas diferencias entre utilizar Gremlin de un programa autónomo y la consola
de Gremlin. Los programas de ejemplo demuestran algunas de esas diferencias. El área
de aplicaciones de ejemplo contiene un ejemplo completo de HTML y Javascript que te
permitirá explorar las 'rutas aéreas' de forma gráfica. La página de inicio para el
proyecto de GitHub incluye un archivo README.md que te ayudará a navegar en el sitio.
Abajo encontrarás algunos recursos que se incluyeron en el libro.

.Donde encontrar el libro, datos y ejemplos
Proyecto::
- https://github.com/krlawrence/graph
Libro Manuscrito en formato Asciidoc::
- Este archivo puede ser visto utilizando la interface web de GitHub. Este siempre
presentará las mas nuevas actualizaciones.
- https://github.com/krlawrence/graph/tree/master/book


.Donde encontrar el libro, ejemplos y datos
Project home::
- https://github.com/krlawrence/graph
Manuscrito del libro en formato Asciidoc::
- Este archivo puede ser visto utilizando la interface de GitHub. Siempre representará la ultima actualización.
- https://github.com/krlawrence/graph/tree/master/book
Instantaneas de los últimos PDFs y HTMLs::
- Esos archivos son regularmente actualizados para reflejar cualquier cambio significativos. Son los únicos formatos generados que se actualizan fuera del ciclo de liberación. La versión PDF incluye paginación, así como numeración por página y es producido utilizando tamaño de página A4. La versión HTML no incluye esas características. Por lo demás son más o ménos idéntico.
- http://kelvinlawrence.net/book/Gremlin-Graph-Guide.pdf
- http://kelvinlawrence.net/book/Gremlin-Graph-Guide.html
Lanzamiento Oficial del libro en múltiples formatos::
- Los lanzamientos oficiales incluyen versiones en Asciidoc, HTML, PDF, ePub, MOBI y DocBook, como instantaneas de todos los ejemplos y otros materiales en un solo paquete. Mi objetivo es tener un lanzamiento oficial al mes proporcionando suficiente material nuevo para justificarlo. Las versiones eBook y MOBI son destinadas para ser leidas utilizando dispositivos de lectura electrónicos y esa es la razón por la cual se utiliza fondo blanco para el resaltado de codigo, para hacer más facil la lectura en dispositivos monocromáticos.
- Recomiendo a medida de lo posible, utilizar la versión PDF, dado que esta tiene numeración de páginas. El formato DocBook puede ser leido utilizando herramientas como Yelp en los sistemas Linux, pero es incluido para que la gente pueda utilizarlo para generar otros formatos que yo no puedo proveer. Actualmente hay un problema con las versiones MOBI y ePub que provoca que los enlaces que contengan texto sean incorrectos. Además de eso, es posible que necesites cambiar el tamaño de la fuente para que puedas leerlo con más facilidad.
- https://github.com/krlawrence/graph/releases
Datos de prueba (`air-routes.graphml`)::
...
- Si deseas saber los cambios que se han realizado a este archivo puedes verlos en el siguiente enlace.
- https://github.com/krlawrence/graph/blob/master/ChangeHistory.md

[[tp34intro]]
Unas palabras acerca de TinkerPop 3.4
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Una actualización importante de Apache TinkerPop, versión 3.4, se lanzó en enero de 2019. Los ejemplos en este libro han sido probados con TinkerPop 3.4 y he hecho un progreso significativo para agregar cobertura de todo lo que se agregó. Sin embargo, el libro aún no incluye la cobertura de cada nueva característica, cambio y desuso introducida con la versión TinkerPop 3.4.


NOTE: El historial de cambios contiene detalles de todo lo que ha sido agregado hasta ahora y se encuentra aquí: 
https://github.com/krlawrence/graph/blob/master/ChangeHistory.md

Tenga en cuenta también que, dado el alcance de la versión 3.4, puede pasar un tiempo antes de que todas las implementaciones de Gráfos de proveedores y de código abierto que admiten Apache TinkerPop se hayan puesto al día con todas estas nuevas características.

NOTE: Los detalles completos de todas las nuevas características agregadas en la versión TinkerPop 3.4 se pueden encontrar en el siguiente enlace:
https://github.com/apache/tinkerpop/blob/master/CHANGELOG.asciidoc

Además de actualizar el libro, comencé un plan continuar agregando de forma incremental la cobertura de estas características a la carpeta `sample-code`. Las pruebas actualmente agregadas incluyen `nested-repeat.groovy` que demuestra el uso de la nueva capacidad de repetir pasos anidados. Se puede cargar y ejecutar desde la consola de Gremlin.

[[whygraph]]
Entonces, ¿qué es una base de datos de Gráfos y por qué debería interesarme?
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Este libro está destinado principalmente a ser un tutorial sobre cómo trabajar con la Base de datos de Gráfos y la tecnología relacionada utilizando el lenguaje de consulta Gremlin. Sin embargo, vale la pena dedicar unos minutos a resumir por qué es importante comprender qué es una base de datos de Gráfos, cuáles son algunos casos de uso adecuados para los Gráfos y por qué debería importarle en un mundo que ya está lleno de todo tipo de SQL y Bases de datos NoSQL. En este libro vamos a discutir 'Gráfos de propiedad dirigida'. A nivel conceptual, estos tipos de Gráfos son bastante simples de entender. Tienes tres bloques de construcción básicos. Vértices (a menudo denominados nodos), aristas y propiedades. Los vértices representan "cosas" como personas o lugares. Las aristas representan conexiones entre esos vértices, y las propiedades son información agregada a los vértices y aristas según sea necesario. La parte de 'dirección' del nombre significa que cualquier arista tiene una dirección. Sale 'de un vértice y' entra 'a otro. Algunas veces escuchará a las personas usar la palabra 'dígrafo' como abreviatura de 'grafo dirigido'. Considere la relación "Kelvin conoce a Jack". Esto podría modelarse como un vértice para cada una de las personas y una ventaja para la relación de la siguiente manera.

[.text-center]
Kelvin -- knows -> Jack

Tenga en cuenta la flecha que implica la dirección de la relación. Si quisiéramos registrar el hecho de que Jack también admite conocer a Kelvin, necesitaríamos agregar una segunda ventaja de Jack a Kelvin. Se podrían agregar propiedades a cada persona para dar más información sobre ellas. Por ejemplo, mi edad podría ser una propiedad en mi vértice.

Resulta que a Jack le gustan los gatos. Es posible que también queramos almacenar eso en nuestro grafo para poder crear la relación:
...
Este es un ejemplo simple, pero espero que ya pueda ver que estamos modelando nuestros datos de la manera en que pensamos sobre ellos en el mundo real. Armado con este conocimiento, ahora tiene todos los componentes básicos que necesita para comenzar a pensar en cómo podría modelar cosas con las que está familiarizado como un gráfos.

Entonces, volviendo a la pregunta "¿por qué debería importarme?", Bueno, si algo parece un gráfos, entonces, ¿no sería genial si pudiéramos modelarlo de esa manera? Muchas cosas en nuestra vida cotidiana se centran en cosas que se pueden representar muy bien en un gráfos. Cosas como sus redes sociales y comerciales, la ruta que toma para llegar al trabajo, las redes telefónicas, las opciones de rutas de aerolíneas para los viajes que necesita tomar son excelentes candidatos. También hay muchas aplicaciones comerciales excelentes para Base de datos de Gráfos y algoritmos. Estos incluyen sistemas de recomendación, prevención del delito y detección de fraude, por nombrar solo tres.

Lo contrario también es cierto. Si algo no parece un gráfico, entonces no intentes forzarlo a serlo. Sus videos probablemente estén funcionando bastante bien en el store de objetos donde los tienes actualmente. Un sistema de contabilidad creado utilizando una base de datos relacional probablemente esté funcionando bien donde está y, de la misma manera, un store de documentos es posiblemente el lugar adecuado para almacenar sus documentos. Por lo tanto, "utilizar la herramienta adecuada para el trabajo" sigue siendo una frase tan válida aquí como en otros lugares. Cuando la Base de datos de Gráfos entra en juego es cuando los datos que están almacenando están intrínsecamente vinculados por su propia naturaleza, la red de rutas aéreas utilizada como base para todos los ejemplos en este libro son un ejemplo perfecto de tal situación.

Aquellos de ustedes que vieron gráfos como parte de un curso de informática son correctos si su reacción fue "Seguramente los gráfos han existido durante siglos, ¿por qué esto se considera nuevo?". De hecho, a Leonard Euler se le atribuye haber demostrado el primer problema gráfico e inventar todo el concepto de "teoría de gráfos" en 1763 cuando investigó el ahora famoso problema "Siete puentes de Koenigsberg".

Si deseas leer un poco más sobre la teoría de gráfos y su aplicación actual, puede encontrar mucha buena información en línea. Aquí hay un enlace de Wikipedia para comenzar: https://en.wikipedia.org/wiki/Graph_theory

Entonces, dada la teoría de gráfos no es una idea nueva, ¿por qué es que recientemente estamos viendo un crecimiento masivo en la construcción e implementación de sistemas y aplicaciones de bases de datos de gráfos? Al menos parte de la respuesta es que el hardware y el software de la computadora han llegado al punto en el que puede construir grandes sistemas de big data que se adaptan bien a un precio razonable. De hecho, es aún más fácil que nunca construir sistemas grandes porque no es necesario comprar el hardware con el que se ejecutará su sistema cuando puedes utilizar la nube.

Si bien ciertamente puede ejecutar una base de datos de gráfos en su computadora portátil, lo hago todos los días, la realidad es que, en la producción, a gran escala, hay enormes sistemas de datos. Los gráfos grandes suelen tener muchos miles de millones de vértices y aristas, ocupando petabytes de datos en el disco. Los algoritmos de gráfos pueden ser intensivos tanto en computación como en memoria, y es solo recientemente que la implementación de los recursos necesarios para sistemas de datos tan grandes ha tenido sentido financiero para usos más cotidianos en los negocios, y no solo en el gobierno o la academia. La base de datos de Gráfos se está adoptando de manera mucho más amplia en todo el espectro, desde investigaciones científicas de alto nivel hasta las redes financieras y más.

Otro factor que realmente ha ayudado a iniciar esta revolución de la base de datos de gráfos es la disponibilidad de tecnología de código abierto de alta calidad. Hay muchos proyectos excelentes de código abierto que abordan todo, desde las bases de datos que necesita para almacenar los datos del gráfico, hasta los lenguajes de consulta utilizados para recorrerlos, hasta mostrar visualmente los gráfos como parte de la capa de interfaz de usuario. En particular, son los llamados 'property graphs' donde vemos un amplio desarrollo y aceptación. En un 'property graph', tanto los vértices como los bordes pueden tener asociadas propiedades (pares clave-valor). Hay muchos estilos de gráfos puedes construir y se han escrito libros completos sobre estos diversos patrones de diseño, pero la tecnología de 'property graphs' en la que nos centraremos en este libro puede admitir todos los patrones de uso más común. Si mientras trabajas en base de datos de gráfos escuchas frases como 'gráfo dirigido' y 'gráfo no dirigido', o 'gráfo cíclico' y 'acíclico', y muchas más, con una búsqueda en internet puedes familiarizarte con esa terminología. Una discusión profunda de estos patrones está más allá del alcance de este libro, y de ninguna manera es esencial tener una formación completa en teoría de grafos para ser productivo rápidamente.

Un tercer y no ménos importante factor en el incremento de adopción de la base de datos de gráfos es el fácil acceso de los programadores. Como verá en los ejemplos de este libro, alguien que quiera experimentar con la tecnología gráfos puede descargar el paquete Apache TinkerPop y, siempre que Java 8 esté instalado, y en tan solo cinco minutos estará en funcionamiento con configuración cero (aparte de descomprimir los archivos). Las Base de datos de Gráfos no obligan a definir esquemas o especificar el diseño de tablas y columnas antes de trabajar y construir gráfos. Los programadores también parecen encontrar estilos gráficos de programación bastante intuitivos, como son modelos cercanos a la forma en que se ve al mundo real.

La tecnología de Bases de datos de Gráfos no deberia ser vista como una tecnología de "quitar y reemplazar", mas bien como un complemento a otras bases de datos ya implementadas. Un caso de uso común es que el gráfo se utilice como una especie de índice inteligente entre otros almacenes de datos. A menudo este tipo de configuración se le llama arquitectura de datos políglota.

[[nodevert]]
Algo sobre la terminología
~~~~~~~~~~~~~~~~~~~~~~~~~~

Cuando hablamos de gráfos, las palabras 'nodo' y 'vértice' son sinónimos. A lo largo de este libro encontrará que se utilizan ambas. Sin embargo, como la documentación de Apache TinkerPop usa casi exclusivamente la palabra 'vértice', tanto como sea posible cuando se discuten las consultas de Gremlin y otros conceptos, trataré de utilizar la palabra 'vértice' o la forma plural 'vértices'. Con la evolución de este libro, me di cuenta de que mi uso de estos términos se había vuelto inconsistente y en futuras actualizaciones planeo, con algunas excepciones, como cuando se trata de árboles binarios, estandarizar el 'vértice' en lugar del 'nodo' para ser consistente con la documentación oficial de TinkerPop.
...
The programming interfaces allow providers of Base de datos de Gráfos to build systems that
are TinkerPop enabled and allow application programmers to write programs that talk
to those systems.

Any such TinkerPop enabled Base de datos de Gráfos can be accessed using the Gremlin query
...
in Capítulo 6 we will take a look at Gremlin Server and some other TinkerPop 3 enabled
...
TinkerGraph. I have arranged the list in dos columns to aid readability.  Don't worry
...
'air-routes' is a model I built of the world airline route nedosrk that is
fairly accurate. 
...
serialization formats. dos common ones are GraphML and GraphSON.  The latter is a
...
include the distance between the dos connected airport vertices as a property called
...
----
 ...
 country (string) : dos letter ISO country code such as US, FR or DE.
...
indexes is not covered in detail until Capítulo 6 "<<beyond>>" . However, as
TinkerGraph does have some indexing capability I have also included some discussion
of it in the "<<tinkerindex>>" section. In Capítulo 6 where I start to look at
...
until Capítulo 6. When working with TinkerGraph there is no need to define a schema
...
NOTE: Capítulo 3 is focussed on queries that simply read from an existing graph. If
you are more interested in adding new vertices, edges and properties or modifying
existing properties you may want to jump to Capítulo 4 and in particular the
"<<addnodes>>" section.

In this Capítulo we will begin to look at the Gremlin query language. I will start
...
a SQL 'join'.  Base de datos de Gráfos by their very nature avoid the need to join things
together (as things that need to be connected already are connected) and this is a
core reason why, for many use cases, Base de datos de Gráfos are a very good choice and can
be more performant than relational databases.                           

Base de datos de Gráfos are usually a good choice for storing and modelling nedosrks.  The
'air-routes' graph is an example of a nedosrk graph. A social nedosrk is of course
another good example. Nedosrks can be modelled using relational databases too but as
you explore the nedosrk and ask questions like "who are my friends' friends?" in a
social nedosrk or "where can I fly to from here with a maximum of dos stops?" things
...
(AUS) to Agra (AGR) with dos stops, we would end up writing a query that looked
something like this:
...
heavily connected data sets like nedosrks, the SQL queries get harder and harder to
...
of SQL and Gremlin is that for data that is very connected, Base de datos de Gráfos provide a
...
table an ID column this is not optional with Base de datos de Gráfos. In some cases the ID
...
accessed using its ID.  Just as with relational databases, Base de datos de Gráfos can be
...
The next dos queries combine the previous dos into a single query. The first one just
...
[source,groovy]
----
// Combining those dos previous queries (dos ways that are equivalent)
...
airports connected to that country directly but the last dos examples are intended to
...
specify dos 'by' modulators. This is because the first and third elements in the path
...
[[edgeexist]]
Does an edge exist between dos vertices?
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
You can use the 'hasNext' step to check if an edge exists between dos vertices and 
...
query below uses an 'as' step to attach a label at dos different parts of the
...
Here is another example of a query that labels dos different parts of a traversal
...
[[exedge]]
Examining the edge between dos vertices
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Sometimes, it is the edge between dos vertices that we are interested in examining
...
the dos examples above could have been written as follows. 
...
[source,groovy]
----
// Return the first dos airport vertices found 
...
The dos examples below will produce the same results.
...
Take a look at the dos examples below.
...
parameter is the label value. The next dos parameters test the properties of all
...
dos columns to aid readability.
...
|between | Between dos values inclusive/exclusive (upper bound is excluded)
...
You will find more examples of these types of queries in the next dos sections.
...
You will notice from the results above that '"Dallas"' appears twice as there are dos
...
Here are dos more examples that use 'without' and 'within' to find routes based on
countries.
...
for both 'Dal' or 'dal' you can do that as shown below using an 'or' step and dos
'has' steps.
...
Only dos results are found. Note that one of the results does contain a vowel but it
...
step. We can see examples of that in the first dos queries below. Note that the first
...
Here is a similar example to the previous dos. We find all of the places you can fly
...
'desc' have been introduced.  All of the examples in the previous dos sections remain
...
enabled Base de datos de Gráfos continue to support the 'incr' and 'decr' keywords. In fact
until the majority of Base de datos de Gráfos offer support for the new 'asc' and 'desc'
...
As you would probably expect, an 'or' step can have more than dos choices. This one
...
query that will produce the same result. Here is an example of the previous dos
...
dos simpler ways of writing this query that do not use 'where' at all are shown below
...
could use the boolean operators within a 'where' step. In this case only dos
...
should see. Note that all the airports returned have dos runways. This is the same
...
There is also a dos parameter form of the 'where' step that you may have noticed used
...
Austin has dos runways so only airports with one runway are returned by this query
when run. 
...
It is also possible to compare dos different properties by adding a second 'by'
...
the same thing in a more simple way, but is does demonstrate dos 'by' modulators
...
country code value of the dos vertices. Lastly a 'select' is used to pick the
...
are comparing the results of dos 'values' steps we do not provide a parameter to the
...
direct flight of the form A->C. In other words it looks for all routes between dos
...
A better way than either of the prior dos combines a 'filter' step with the 'where'
and 'select' steps. 
...
have a route back to JFK. Note that the 'as' step plays dos roles in this query.
...
results of dos traversals but you can certainly combine more as needed. Note that the
...
using dos 'has' steps both inside a single 'union' step. This does however
...
The previous dos queries used a 'path' step to essentially show each individual
...
We can put more than dos traversals inside of a 'coalesce' step. In the following
...
[[optional]]
Returning one of dos possible results - introducing 'optional'
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Sometimes it may be useful to return one of dos results depending upon the outcome of
...
queries. We can use a 'repeat...until' loop to look for paths between dos airports
...
example we want to next try five hops instead of the current dos..
...
runs, is essentially equivalent to the 'times' step. Take a look at the dos queries
...
NOTE: If you simply want to find out if any route exists between dos airports, there
...
The paths returned show a selection of ways to get to Miami from Austin with dos
stops but none of the results show less than dos stops. Is this what we really
wanted?
...
saying "at most three hops" or in airline terms "at most dos stops".
...
follow. In the example below, we start in Austin and go out dos hops using a 'repeat'
...
of where the cycle is not back to the starting vertex. The dos results below were
...
that even though we are adding dos integers together, the result comes back as a
...
or dos runways, our result looks pretty reasonable. Clearly the few airports with six,
...
traversal steps. They will only work using Base de datos de Gráfos that allow Groovy code to
...
The next queries show dos ways of finding airports with 6 or more runways.
...
Next, let's look at dos ways of finding flights from airports in South America to
...
Here are dos more queries that look for things that meet a specific criteria. The
...
This query looks for any airports that have an elevation above 10,000 feet. dos ways
...
Keeping information in dos places within the same graph
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
to make a point. Look at the following dos queries that return the same
...
If we were building a graph of a social nedosrk that included famous people we might
have to worry. Consider some of the people on Twitter with millions of followers.
Without taking some precautions, such a social nedosrk, modelled as a graph, could
face issues.
...
By way of a simple example, let's define a method that will tell us how far apart dos
airports are and then invoke it.

[source,groovy]
----
// A simple function to return the distance between dos airports
def dist(g,from,to) {
...
dos examples are shown below. One uses the 'addOutE' step and the other uses the
...
above used LONG values. However, as one example, some Base de datos de Gráfos that do allow
...
a list be created. Look at the dos examples below. They do not produce the same
results.
...
g.addV().property('one','hi').
         property('one','hello').
         property('dos','goodbye').
         property('one','hello again').
         valueMap()

[one:[hi,hello,hello again],dos:[goodbye]]
----
...
between the dos forms. I have truncated the output to keep things simple.

Here is what Gremlin shows us for the first query when we use an 'explain' step.
As you can see our query has been compiled into an 'AddVertexStep' with dos
...
Final Traversal  [TinkerGraphStep(vertex,[v[54800]]),
                    ...
                    AddPropertyStep({value=[goodbye], key=[dos]}
...
[source,groovy]
----
g.addV('test').property('dups','one').property('dups','dos').property('dups','one')
...
[source,groovy]
----
g.V().hasLabel('test').property(list,'dups','dos')

g.V().hasLabel('test').valueMap()

[dups:[one,dos,one,dos]]
----
...
will add a property called 'comment' to vertex 'V(3)' and also add dos meta
...
of dos values. The 'union' step contains an 'identity' step to indicate that we want
...
dos keys. The first, called 'dfw', will contain the vertex for the DFW airport and
...
As we shall see in the next dos sections, sometimes it is necessary to use the
...
Using 'local' scope with a 'limit' step we can extract just the first dos entries
from the list.
...
Likewise, using 'local' scope with a 'tail' step we can extract just the last dos
...
example below uses a 'project' step to feed the 'math' step with dos values
...
The dos queries shown below create maps of airport and runway key/value pairs for all
...
Given these dos collections, we might want to divide the sum of one set of values by
...
This time, as the Austin (AUS) airport has dos runways our calculation in effect
became '2 + runways'.
...
part of the TinkerPop Java Enum called 'Operator'. dos such operators that we
...
distance of the dos hops. This is a perfect example of a query where using a 'sack'
...
sum of the dos prior route distances. So, for example, we can see that the total
...
the query was tailored to produce results for a dos hop query. We could of course
...
NOTE: For reasons of security not all Base de datos de Gráfos, especially those that are
...
Lambda as shown in the following dos examples. The examples take advantage of methods
...
shorthand form of a 'map' step. The following dos queries yield the same results for
...
this book, is 'flatMap'. The dos steps are similar but have a fundamental difference
...
called 'f', that tests the dos values passed in to see if 'x' is greater than twice
'y'. This new predicate can then be used as part of a 'has()' step by using it as a
parameter to the 'test()' method.  When 'f' is called, it will be passed dos
...
sometimes referred to was a 'dos-arity' predicate as it takes dos parameters. We
...
comparison between dos objects and returns a simple true or false result. Like
...
with the Austin vertex but this time going out dos hops. We achieve this using a
'repeat' step.
...
TinkerPop 3 offers dos different JSON packaging options. These are not to be confused
...
time. I am just going to focus on dos of them, namely, 'clock' and 'clockWithResult'.
...
takes dos parameters. The first is an integer telling it how many times to run the
...
(AUS) and Los Angeles (LAX) with zero, one, dos or three stops. The query avoids any
...
Looking for routes with exactly dos stops takes quite a bit longer but is still
fairly fast.
...
overall performance of your queries. dos methods 'createIndex' and 'dropIndex' are
...
The timing difference between the dos queries can be attributed to the index. In the
...
When discussing graph processing dos terms regularly come up. These terms are Online
...
In this Capítulo you will find more Gremlin queries that operate on the 'air-routes'
...
The previous query is a slightly shorter form of the dos queries below which would
...
airport with dos runways, in practice the second runway is primarily used as a
...
dos character ISO country code and the value is the number of airports that country
has.
...
to return our dos variables as the results of the query.
...
found six airports that have six or more runways, 3079 that have dos or less and
...
An example of a common question we might want to answer with a nedosrk graph, of
...
outgoing route. We can see that 654 airports have just dos routes and so on. We can
...
out the distribution of incoming routes. Remembering that in an air route nedosrk
...
Here is a twist on the above theme. How many places can I get to from London in dos
...
[[doesanyrouteexist]]
Does any route exist between dos arirports?
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

We have already looked at different ways to discover shortest routes (by hops)
between dos airports using queries such as the one below. This query looks for a
...
The difference between the dos queries is the addition of a 'dedup' step. This
...
airport codes and the distances. Notice the use of dos 'by' modulators in this query
to decide which values are returned from the source vertex, the edge and the
destination vertex respectively. Also note that only dos were specified but three
...
pair twice.  In other words the distance between dos airports in just one direction
...
Calculating the distance between dos directly connected airports is very easy. All we
...
It turns out that just dos airports in the graph are located that far North as shown
...
specific airport anywhere in the journey. Let's build an example in dos stages. First
...
dos queries shown below use the label from the DFW Airport vertex as the label for a
new vertex.
...
earlier in Capítulo 4, a 'property' step returns the vertex that the property was
...
seven hops and found that all but dos of the airports were able to reach LHR
...
This left me with an interesting conclusion. There must be dos airports in the graph
that are not orphans, in other words they have some routes, but they are isolated
from the main nedosrk. So I needed a query to find them which is the subject of the
next section. What the queries in this section did prove is that from any airport in
the graph, with the exception of the dos cases we need to investigate further, London
...
[[mininedosrks]]
Finding isolated sub-nedosrks
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

As discussed above, while I was working on the queries for the previous section I
realized that there are dos airports in the graph that are not true orphans, as they
do have routes between each other but that they are part of a small nedosrk that is
not itself connected to the main route nedosrk. In other words, the nedosrk is an
isolated mini nedosrk within the main graph and you can never get to London from
...
isolated nedosrks within the main graph exist. However, these are not due to errors
...
any dos airports you run the risk of writing a query that runs for an extremely long
...
edge between the same dos vertices in the same direction.  Most property graph
...
for there to be more than one edge between the same dos airports in the same
...
[[flr]]
Finding the longest flight route between dos adjacent airports in the graph
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
maximum length between any dos airports. It turns out that this simple looking query
...
dos airports don't count as different routes for this example).
...
add more experiments to it. If we add the dos lines below we can extract the city
...
Let's now add something a bit more interesting to our program. The following dos
...
provide. The list is formatted in dos columns to aid readability.
...
[source,groovy]
----
n = g.addV().property('mylist','one').property('mylist','dos').next()
...
can exist between dos vertices.
...
store the distance between dos airports.
...
JanusGraph supports dos different types of indexing known as 'graph indexes' and
...
that the only differences from the prior example are that we add dos keys rather than
...
// Do any points in the dos circles intersect?
...
http://www.oracle.com/technedosrk/database/database-technologies/berkeleydb/overview/index.html
...
building and interacting with, Base de datos de Gráfos.
...
closed you will have to reconnect using one of the dos 'open' steps shown above before
...
what we have now is a nice collection of lists, each one containing dos strings and
...
The query below returns a path between dos airports as a list of airport codes.
...
returned is the dos vertices along with their IDs, labels and properties.
...
Path with dos vertices and an edge
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
The following path query returns a list containing dos strings representing airport
...
especially when vertices contain lots of properties is to use dos CSV files. One will
contain all of the vertex data and the other will contain all of the edge data.

[[csvair]]
Using dos CSV files to represent the air-routes data
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
