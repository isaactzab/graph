...
bases de datos de grafos y el lenguaje de consulta de Gremlin con sugerencias, consejos 
...
partes mientras utilizaba una base de datos de grafos y especialmente mientras utilizaba
...
el lenguaje traversal para trabajar con bases de datos de grafos.
...
traversal, por medio de ejemplos realistas y características que los datos de grafos
...
- Se comienza haciendo una breve recapitulación de por qué nos interesa una Base de datos de Grafos y se discute unos buenos casos de uso. También se provee de enlaces a programas de prueba y otros recursos adicionales referenciados por este libro.
...
Capítulo 4 - BEYOND BASIC QUERIES::
...
Tenga en cuenta también que, dado el alcance de la versión 3.4, puede pasar un tiempo antes de que todas las implementaciones de gráficos de proveedores y de código abierto que admiten Apache TinkerPop se hayan puesto al día con todas estas nuevas características.
...
[[whygraph]]
Entonces, ¿qué es una base de datos de gráficos y por qué debería interesarme?
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

This book is mainly intended to be a tutorial in working with Base de datos de Grafos and
...
Things such as your social and business nedosrks, the route you take to get to work,
the phone nedosrk, airline route choices for trips you need to take are all great
candidates. There are also many great business applications for Base de datos de Grafos and
...
tool for the job" remains as valid a phrase here as elsewhere. Where Base de datos de Grafos
come into their own is when the data you are storing is intrinsically linked by its
very nature, the air routes nedosrk used as the basis for all of the examples in
...
government or academia. Base de datos de Grafos are becoming much more broadly adopted across
the spectrum, from high-end scientific research to financial nedosrks and beyond.
...
with Base de datos de Grafos, a quick online search will get you to a place where you can get
...
little as five minutes.  Base de datos de Grafos do not force you to define schemas or
...
The programming interfaces allow providers of Base de datos de Grafos to build systems that
are TinkerPop enabled and allow application programmers to write programs that talk
to those systems.

Any such TinkerPop enabled Base de datos de Grafos can be accessed using the Gremlin query
...
a SQL 'join'.  Base de datos de Grafos by their very nature avoid the need to join things
together (as things that need to be connected already are connected) and this is a
core reason why, for many use cases, Base de datos de Grafos are a very good choice and can
be more performant than relational databases.                           

Base de datos de Grafos are usually a good choice for storing and modelling nedosrks.  The
...
of SQL and Gremlin is that for data that is very connected, Base de datos de Grafos provide a
...
table an ID column this is not optional with Base de datos de Grafos. In some cases the ID
...
accessed using its ID.  Just as with relational databases, Base de datos de Grafos can be
...
enabled Base de datos de Grafos continue to support the 'incr' and 'decr' keywords. In fact
until the majority of Base de datos de Grafos offer support for the new 'asc' and 'desc'
...
traversal steps. They will only work using Base de datos de Grafos that allow Groovy code to
...
above used LONG values. However, as one example, some Base de datos de Grafos that do allow
...
NOTE: For reasons of security not all Base de datos de Grafos, especially those that are
...
building and interacting with, Base de datos de Grafos.
